...
func guardar_datos_json(datos_a_guardar:Dictionary)->int:
	...
	var resultado: int = archivo.open(seleccionar_ruta(JUEGO),File.WRITE)
	if resultado == OK:
...
func guardar_datos_tres(datos_a_guardar:Dictionary)->int:
	...
	var resultado : int= ResourceSaver.save(seleccionar_ruta(JUEGO),datos)
...
func cargar_datos_json()-> Dictionary:
	...
	var resultado: int = archivo.open(seleccionar_ruta(JUEGO),File.READ)
	if resultado== OK:
...
func cargar_datos_tres()-> Dictionary:
	var datos:Resource=load(seleccionar_ruta(JUEGO))
	return{
		"nombre": datos.nombre
	}
...
func cargar_datos_juego()->void:
	...
	DatosJuego.monedas_oro=datos.moneda_oro
